...
def cosine_similarity_pair(
    ...
    *,
    ...
    if not np.isfinite(intensity_exponent) or intensity_exponent <= 0:
        raise ValueError("intensity_exponent must be positive")

    if not isinstance(max_cum_peaks, (int, np.integer)) or max_cum_peaks <= 0:
...
def cosine_similarity_by_key(
    ...
    *,
    ...
    unique within each dataset; duplicates are rejected because they would
    create one-to-many or many-to-many pairings. Missing and unmatched values
    are ignored.
    ...
    reserved = {"index1", "index2", "cosine_similarity"}
    if key1 in reserved or key2 in reserved:
        raise ValueError("Key column names must not collide with result columns: "
                         "index1, index2, cosine_similarity")

    left = pd.DataFrame({
        key1: ds1[key1].reset_index(drop=True),
        "index1": np.arange(ds1.n_rows, dtype=np.int64),
    }).dropna(subset=[key1])
    right = pd.DataFrame({
        key2: ds2[key2].reset_index(drop=True),
        "index2": np.arange(ds2.n_rows, dtype=np.int64),
    }).dropna(subset=[key2])

    for side, key, values in (("ds1", key1, left), ("ds2", key2, right)):
        duplicate = values[key].duplicated(keep=False)
        if duplicate.any():
            examples = values.loc[duplicate, key].drop_duplicates().tolist()[:5]
            raise ValueError(f"{side} key column {key!r} contains duplicate values; "
                             f"one-to-one matching is ambiguous; examples: {examples}")
