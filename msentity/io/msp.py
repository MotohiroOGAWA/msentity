...
def read_msp(
    ...
    *,
    ...
    """
    Read an MSP file into an MSDataset.
...
def write_msp(
    ...
    *,
    ...
    headers
        ...
        columns are written except ``"NumPeaks"``.
    header_map
        Optional mapping from dataset column names to MSP header names.
    ...
    # Resolve spectrum-level headers
    if headers is None:
        ...
        with open(path, "w", encoding=encoding) as wf:
            for record in dataset:
                try:
                    ...
                    # Collect peak rows first so we can determine which peak metadata
                    # columns are actually used in this record.
                    ...
                    # Write peak header only if at least one peak metadata column is used
