...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#remove-dataset').isDisabled(), true);
    ...
    await page.locator('#dataset-select').selectOption('original'); assert.equal((await last()).datasetId, 'original'); await datasetPage('original');
    await page.locator('#dataset-select').selectOption('added'); await datasetPage('added');
    await page.locator('#remove-dataset').click(); assert.equal((await last()).type, 'remove-dataset');
